...
| `Verbosity`  | Supported | Supported |  |
| `ReasoningEffort`  | Supported | Supported |  |
...
### `Verbosity` — Verbosity of generated output (GPT-5)

`"auto"` (default) | `"low"` | `"medium"` | `"high"`


Verbosity of the output generated by the GPT-5 family of models, specified as `"auto"`, `"low"`, `"medium"`, or `"high"`.

If `Verbosity` is set to `"auto"`, then the software chooses the default value of the model used to generate text, for example:

| **Model**   | **Default Verbosity** |
| :-- | :-- |
| GPT-5   | `"medium"`   |
| GPT-5.1   | `"medium"`   |
| GPT-5.2   | `"medium"`   |

For models that do not have a verbosity parameter, this name-value argument has no effect.

This option is only supported for [`openAIChat`](openAIChat.md) and [`azureChat`](azureChat.md) objects.

### `ReasoningEffort` — Amount of internal reasoning (GPT-5)

`"auto"` (default) | string | char


Amount of internal reasoning generated by the GPT-5 family of models, specified as a string scalar of character array.

If `ReasoningEffort` is set to `"auto"`, then the software chooses the default value of the model used to generate text. The supported values and default value depend on the model, for example:

| **Model**   | **Default Verbosity** | **Supported Values** |
| :-- | :-- | :-- |
| GPT-5   | `"medium"`   | `"minimal"`, `"low"`, `"medium"`, `"high"` |
| GPT-5.1   | `"none"`   | `"none"`, `"low"`, `"medium"`, `"high"` |
| GPT-5.2   | `"none"`   | `"none"`, `"low"`, `"medium"`, `"high"`, `"xhigh"` |

For models that do not have a reasoning effort parameter, this name-value argument has no effect.
